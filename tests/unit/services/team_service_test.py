...
class TestsTeamService:
    ...
    def test_create_team_should_give_an_error_for_duplicate_team(self):
        ...
        mock_dynamo_db = MagicMock()
        mock_dynamo_db.delete_item.return_value = existing_team
    ...
    def test_delete_team_should_remove_from_repository(self):
        ...
        mock_dynamo_db = MagicMock()
        mock_dynamo_db.delete.return_value = True

        service = TeamService(mock_dynamo_db)

        result = service.delete_team(existing_team_name)

        mock_dynamo_db.delete.assert_called_with(existing_team_name)
        assert result
